...
export function UpcomingStreamCard({ stream, sectionName, isReminderSet, onToggleReminder }: UpcomingStreamCardProps) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-bg overflow-hidden transition-all duration-200 hover:border-[#8B5CF6]/40 hover:shadow-[0_0_24px_rgba(139,92,246,0.1)]">
      {/* Preview */}
      <div className="relative aspect-video bg-gradient-to-br from-bg via-bg to-[#1a1330] flex items-center justify-center overflow-hidden">
        {stream.thumbnail_url ? (
          <img src={stream.thumbnail_url} alt="" className="absolute inset-0 w-full h-full object-cover" />
        ) : (
          <ClockIcon className="h-9 w-9 text-border" />
        )}
      ...
      <div className="p-3.5">
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium text-text-primary truncate">{stream.host_username}</p>
        ...
        <h3 className="text-sm font-semibold text-text-primary line-clamp-1">{stream.title}</h3>
          ...
          <button
            ...
            className={`flex items-center gap-1 text-[11px] font-medium px-2 py-1 rounded-lg transition ${
              isReminderSet
                ...
                : 'text-text-secondary hover:text-[#00C8FF] hover:bg-glass'
            }`}
          >
