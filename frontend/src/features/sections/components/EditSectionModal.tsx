// frontend/src/features/sections/components/EditSectionModal.tsx
...
/** Edits a section's own details (name/course/year level/academic year/
 * description) - reuses the existing updateSection mutation (already backed
 * by PUT /sections/{id}, already wired in useSections()) which previously
 * had no UI calling it. Deliberately does not touch subjects/schedules
 * (AddSubjectModal/EditTeachingAssignmentModal own that) or section
 * membership - only the same fields CreateSectionModal collects on create. */
export default function EditSectionModal({ section, onClose }: EditSectionModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateSection({ id: section.id, data: formData });
      onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
      <div className="w-full max-w-md rounded-2xl border border-[#1E3447] bg-[#111E2B] shadow-[0_0_40px_rgba(0,200,255,0.06)] p-6 max-h-[90vh] overflow-y-auto themed-scrollbar">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-[#F1F5F9]">Edit Section Details</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#94A3B8] mb-1.5">Section Name *</label>
            <input
              type="text"
              value={formData.name}
          ...
          <div>
            <label className="block text-sm font-medium text-[#94A3B8] mb-1.5">Course</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-1.5">Year Level</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-1.5">Academic Year</label>
              <input
                type="text"
