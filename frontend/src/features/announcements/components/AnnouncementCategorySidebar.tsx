  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-glass backdrop-blur-xl p-4">
        ...
        {popular.length === 0 ? (
          <p className="text-xs text-text-muted py-4 text-center">No announcements yet.</p>
        ) : (
          <div className="space-y-1">
            {popular.map((a) => (
              <button
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-text-muted mt-0.5">{formatDate(a.created_at)}</p>
                </div>
                {a.image_url && (
                  <img src={a.image_url} alt="" className="h-9 w-9 rounded-lg object-cover flex-shrink-0 border border-border" />
                )}
              </button>
            ))}
          </div>
