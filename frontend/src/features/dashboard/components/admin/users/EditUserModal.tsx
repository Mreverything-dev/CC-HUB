...
/** General profile fields only (username/email/name) - role, section, and
 * account status are handled by their own dedicated actions (Change Role,
 * Suspend/Activate) elsewhere in this menu, not duplicated here. */
export default function EditUserModal({ user, isLoading, onConfirm, onClose }: EditUserModalProps) {
  // AdminUserListItem only carries a single joined "full_name" (first +
  // last from whichever profile table matches the user's role) - split it
  // back into two fields for editing, first word vs. the rest.
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[60]" onClick={onClose}>
      <div
        className="w-full max-w-md rounded-2xl border border-[#1E3447] bg-[#111E2B] shadow-[0_0_40px_rgba(0,200,255,0.06)] p-4 sm:p-6 max-h-[90vh] overflow-y-auto themed-scrollbar"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-[#F1F5F9]">Edit User</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-1.5">First Name</label>
              <input type="text" value={first} onChange={(e) => setFirst(e.target.value)} className={inputClassName} />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-1.5">Last Name</label>
