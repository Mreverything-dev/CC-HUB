  ...
  return (
    <div className="rounded-2xl border border-border bg-glass backdrop-blur-xl p-4 sm:p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-text-primary">Sections Overview</h3>
        <span className="text-[10px] text-text-muted uppercase tracking-wide">By enrollment</span>
      </div>
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-text-muted py-6 text-center">No sections yet</p>
      ) : (
        <div className="space-y-1">
          {ranked.map((s) => (
            <div key={s.id} className="flex items-center gap-3 px-1.5 py-2 rounded-xl hover:bg-glass transition">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#00C8FF]/10 text-[#00C8FF] flex-shrink-0">
                <UserGroupIcon className="h-4 w-4" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-sm text-text-primary font-medium truncate">{s.name}</p>
                <p className="text-xs text-text-muted truncate">{s.member_count} member{s.member_count === 1 ? '' : 's'}</p>
              </div>
            </div>
          ))}
        </div>
