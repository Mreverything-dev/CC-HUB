...
function SectionLabel({ icon: Icon, label }: { icon: React.ComponentType<{ className?: string }>; label: string }) {
  return (
    <p className="flex items-center gap-1.5 px-3 pt-2.5 pb-1 text-[11px] font-semibold uppercase tracking-wide text-text-muted">
...
export function GlobalSearchDropdown({
  ...
  return (
    <div className="absolute left-0 right-0 top-full mt-2 rounded-2xl border border-border bg-bg shadow-2xl max-h-[70vh] overflow-y-auto themed-scrollbar z-50">
            ...
            <div>
              ...
              {people.map((p) => (
                <button
                  ...
                  className="w-full flex items-center gap-2.5 px-3 py-2 hover:bg-glass transition text-left"
                >
                  <Avatar src={p.avatar_url} name={personName(p)} size="sm" />
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-text-primary truncate">{personName(p)}</p>
            ...
            <div>
              ...
              {announcements.map((a) => (
                <button
                  ...
                  className="w-full flex items-start gap-2.5 px-3 py-2 hover:bg-glass transition text-left"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-text-muted truncate">{formatRelativeTime(a.created_at)}</p>
                  </div>
                </button>
              ))}
            </div>
              ...
              {posts.map((post) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-text-secondary truncate">{post.content}</p>
                  </div>
                </button>
              ))}
            </div>
