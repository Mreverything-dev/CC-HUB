...
import { BookOpenIcon, ClockIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
/**
 * Always-on "today's classes" status card (next class + countdown, or a
 * no-classes empty state) - no rotating welcome message. Clicking it (once
 * there's at least one class today) opens the full today's schedule in a
 * lightweight modal, reusing this same `entries` list rather than fetching
 * or computing anything new.
 */
export function ClassReminderCard({ scheduleLabel, entries, nextUpcoming }: ClassReminderCardProps) {
